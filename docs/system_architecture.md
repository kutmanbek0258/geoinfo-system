...
| **Печать отчетов** | **Matplotlib / ReportLab (воркер), Spring Kafka (оркестрация)** | Асинхронный экспорт векторных объектов по ID, рендеринг растровых тайлов через TiTiler и генерация PDF-отчетов. |
...
Для создания тяжелых картографических PDF-документов (форматов A4-A0) без блокировки пользовательского интерфейса используется распределенный асинхронный конвейер на базе **Spring Kafka**, **Matplotlib**, **ReportLab** и **TiTiler**:

1. **Инициация задачи:** Фронтенд отправляет спецификацию отчета (метаданные, формат, BBox, растровые слои и массивы идентификаторов векторных объектов `pointIds`, `multilineIds`, `polygonIds`) методом `POST` на эндпоинт `/api/print/tasks` в `geoprint-service`.
2. **Оркестрация экспорта векторов (Java):** 
   * `geoprint-service` (Java) временно приостанавливает запуск генерации PDF, помечая ссылки слоев как `"export-pending:..."`.
   * Сервис отправляет асинхронные запросы `GeoVectorExportRequest` в топик **`geo.vector.export`**.
   * Микросервис `geodata-service` считывает запросы, выгружает соответствующие объекты по ID из PostgreSQL/PostGIS, собирает их в единый GeoJSON-файл и сохраняет его в S3 (MinIO).
   * После загрузки `geodata-service` публикует ответ со ссылкой S3 в топик **`geo.vector.export.results`**.
3. **Ожидание и Запуск воркера:**
   * `geoprint-service` принимает результаты экспорта векторов (группа `geoprint-vector-results-group`) и заменяет временные ссылки слоев в БД на полученные ссылки S3 (`s3://...`).
   * Как только все векторные слои успешно экспортированы, `geoprint-service` отправляет задачу в топик **`geo.print.tasks`**, инициируя работу Python-воркера.
4. **Рендеринг карты (Python-воркер):**
   * Воркер `geoprint-worker` считывает задачу, скачивает GeoJSON-файлы векторных слоев из S3 и загружает их в оперативную память.
   * Растровые слои (`COG`) запрашиваются у **TiTiler** через REST API (`/cog/bbox/...`) с наложенными стилями (`colormap`) и интерполяцией (`resampling`).
   * Воркер накладывает растровые PNG и векторные объекты (посредством `geopandas` с наложением стилей `layerStyle`) на холст Matplotlib в проекции карты и экспортирует ее в PNG с высоким DPI.
5. **Сборка PDF и Завершение:**
   * Воркер генерирует PDF-страницу нужного размера (A4-A0) через библиотеку **ReportLab**, размещает на ней метаданные (заголовок, автор, организация, дата) и нарисованную карту.
   * Готовый PDF-отчет выгружается в S3-бакет `geo-reports`. Воркер сообщает об успешном выполнении в топик `geo.print.results`.
   * `geoprint-service` ловит событие завершения, переводит задачу в статус `COMPLETED` и генерирует временный Presigned URL для скачивания файла клиентом.
