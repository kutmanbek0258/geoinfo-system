...
Загрузка картографических данных (GeoTIFF) с дрона в систему администратором. Загруженные файлы проходят автоматическую верификацию и конвертируются в оптимизированный для облака формат Cloud Optimized GeoTIFF (COG), после чего сохраняются в S3 (MinIO).

Сервис TiTiler отвечает за динамическое чтение COG-файлов из MinIO и публикацию растровых слоев по протоколу XYZ тайлов.

Хранение метаданных о дроновых полетах (дата съемки, охват, разрешение, CRS, BBox) в основной базе данных.

Возможность удаления загруженных слоев дроновых снимков через бэкенд, который также очищает соответствующие файлы в S3.
...
ФТ.2.10. Загрузка дроновых снимков: Система должна позволять загружать картографические данные (GeoTIFF) с дрона. Загруженные файлы проходят автоматическую верификацию и конвертируются в формат COG, после чего раздаются по протоколу XYZ через TiTiler.
...
НФТ.1.4. Обработка дроновых снимков: Процесс оптимизации (COG) и публикации нового слоя через TiTiler для одного ортофотоплана размером до 1 ГБ должен занимать не более 10 минут.
...
TiTiler: Динамическая обработка и предоставление тяжелых растровых данных (Cloud Optimized GeoTIFF) в виде XYZ тайлов.
...
Сервер геоданных: TiTiler (растры XYZ/COG) и pg_tileserv (векторы MVT).
...
Сервис дроновых снимков (Geo Abstraction Service): Управление метаданными снимков, интеграция с TiTiler и управление COG-растрами и Cesium-рельефом. База данных: PostgreSQL.
...
5.3. Разработка
Инфраструктура: Настройка Docker Compose для всех компонентов (включая TiTiler, OnlyOffice, MinIO, Kafka). Разработка API Gateway.
...
TiTiler: Современный легковесный сервер тайлов, предназначенный для динамической публикации растровых данных в формате Cloud Optimized GeoTIFF (COG) напрямую из S3.
