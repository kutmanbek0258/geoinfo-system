...
- `nginx-proxy`: Обратный прокси, балансировщик нагрузки и кэширующий слой.
...
- `stream-service`: Сервис для управления потоковыми трансляциями (например, с камер IP-камер).
- `geoabstraction-service`: Оркестратор для обработки растровых данных и рельефа. Включает планировщик очистки (Garbage Collector).
- `frontend`: Клиентское SPA-приложение (Vue.js), обслуживаемое через Nginx.

#### 3. Воркеры обработки (Python)
- `geoabstract-worker`: Обработка спутниковых снимков (Sentinel-2, Landsat 8), создание COG и расчет индексов.
- `terrain-worker`: Генерация 3D-рельефа (quantized-mesh) из DEM-данных.
- `geoanalysis-worker`: Выполнение тяжелых пространственных операций (буферы, изолинии, зональная статистика). — *В стадии активной разработки*
...
- `postgres-geoabstraction`: База данных PostGIS для `geoabstraction-service`.
...
- `titiler`: Сервер для публикации и раздачи геоданных (Cloud Optimized GeoTIFF) в формате XYZ тайлов.
...
## Очистка данных и Garbage Collection

Система включает механизмы автоматического поддержания чистоты хранилищ:

1.  **Startup Cleanup:** При запуске контейнеров `geoabstract-worker` и `terrain-worker` автоматически очищают свои временные директории в `/tmp`, удаляя артефакты от предыдущих сессий.
2.  **MinIO Cleanup:** Исходные файлы (архивы) удаляются из MinIO сразу после завершения обработки (как успешной, так и неудачной).
3.  **Scheduled GC:** Сервис `geoabstraction-service` ежедневно в 03:00 выполняет полную синхронизацию БД и файловых хранилищ, удаляя физические файлы и папки, для которых нет соответствующих записей в базе данных.
4.  **Multi-stage Deletion:** Удаление слоя пользователем инициирует цепочку событий: отправка Kafka-события воркерам -> удаление физического файла из MinIO -> удаление записи из БД.
...
Для быстрого старта в этом режиме созданы специальные файлы:
- `.env.dev`: Конфигурация с предустановленными портами для прямого доступа.
- `.env.secrets.dev`: Секреты с паролями по умолчанию для разработки.

**Команда для запуска:**
```shell
docker-compose --env-file .env.secrets.dev --env-file .env.dev -f docker-compose.yml -f docker-compose.dev.yml up --build
```
- Используя эти файлы, вы получаете доступ к сервисам напрямую (например, фронтенд на `http://localhost:8080`, API Gateway на `http://localhost:9005`).
...
- `${TITILER_PORT}`: TiTiler (только в среде разработки, порт 8085; в продакшене доступен через nginx-proxy по пути /raster/cog/).
...
- **MinIO (растровые слои):** Растровые данные (COG) сохраняются напрямую в объектное хранилище MinIO в бакете `geo-abstraction-input`. Использование локального диска для монтирования файлов в гео-сервисы больше не требуется.
...
-   **Учетные данные и секреты инфраструктурных сервисов**: `ROOT_USER`/`PASSWORD` для MinIO, `JWT_SECRET` для OnlyOffice. (Секреты GeoServer больше не требуются).
