...
- **Визуализация 3D-моделей и CityGML (3D Tiles):** Импорт и обработка трехмерных моделей объектов и городских территорий в форматах OBJ, ZIP и CityGML (`.gml`, `.xml`).
    - Автоматическая геопривязка и расчёт ECEF-матриц геопозиционирования (`root.transform`).
    - Конвертация моделей в стандарт **3D Tiles** (`.b3dm` + `tileset.json`) для отображения на глобусе Cesium.
    - Оптимизированная генерация текстурных атласов (Texture Atlas) с возможностью отключения импорта растров.
    - Выделенный раздел управления 3D-слоями (`/ui/3dtiles`) с управлением видимостью и быстрым фокусированием (`zoomTo`).
