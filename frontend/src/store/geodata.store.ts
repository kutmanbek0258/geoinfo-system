...
interface GeodataState {
    ...
    isLoading: boolean;
    error: string | null;
    activeCameraStream: { geoObjectId: string, hlsStreamUrl: string } | null;
}
...
const mutations = {
    ...
    SET_ACTIVE_CAMERA_STREAM(state: GeodataState, payload: { geoObjectId: string, hlsStreamUrl: string } | null) {
...
const actions = {
    ...
    async startCameraStream({ commit, dispatch }: ActionContext<GeodataState, any>, geoObjectId: string) {
        ...
        commit('SET_ERROR', null);
        try {
            const hlsStreamUrl = await streamService.startStream(geoObjectId);
            commit('SET_ACTIVE_CAMERA_STREAM', { geoObjectId, hlsStreamUrl });
        } catch (err) {
