...
async function runAnalysis() {
  ...
  try {
    const inputs: Record<string, AnalysisDataSource> = {
      'classified_raster': formData.value.rasterSource
    };

    const dto: CreateAnalysisTaskDto = {
      pluginName: 'polygonize_raster',
    ...
    console.error('Failed to run polygonize raster analysis:', err);
  } finally {
    loading.value = false;
  }
}
  ...
  <v-dialog v-model="internalShow" max-width="500px">
    <v-card>
      <v-card-title class="pa-4 bg-primary text-white">
        <v-icon start icon="mdi-vector-polygon"></v-icon>
        Векторизация растра (Polygonize)
      </v-card-title>
      ...
      <v-card-text class="pa-4">
        <v-form ref="form" v-model="valid">
          <v-select
            ...
            :items="rasterOptions"
            label="Исходный классифицированный растр"
            variant="outlined"
            density="comfortable"
            required
          ...
          <v-select
            v-model.number="formData.parameters.connectivity"
            :items="[
              { title: '4-связная (по смежным сторонам)', value: 4 },
              { title: '8-связная (по сторонам и углам)', value: 8 }
            ]"
            label="Связность пикселей"
            variant="outlined"
            density="comfortable"
            required
          ></v-select>
          ...
          <v-select
            v-model="formData.parameters.format"
            :items="[
              { title: 'GeoJSON', value: 'GeoJSON' },
              { title: 'GeoPackage (GPKG)', value: 'GPKG' }
            ]"
            label="Выходной формат вектора"
            variant="outlined"
            density="comfortable"
            required
          ></v-select>
          ...
          <v-checkbox
            v-model="formData.parameters.mask_zero"
