...
async function runAnalysis() {
  ...
  try {
    ...
    console.error('Failed to run hillshade analysis:', err);
  } finally {
    loading.value = false;
  }
}
  ...
  <v-dialog v-model="internalShow" max-width="500px">
    <v-card>
      <v-card-title class="pa-4 bg-primary text-white">
        <v-icon start icon="mdi-brightness-6"></v-icon>
        Генерация теневой отмывки
      </v-card-title>
      ...
      <v-card-text class="pa-4">
        <v-form ref="form" v-model="valid">
          ...
          <v-row>
            <v-col cols="6">
              <v-text-field
                v-model.number="formData.parameters.azimuth"
                label="Азимут (°)"
                type="number"
                min="0"
                max="360"
                ...
                :rules="[
                  ...
                  v => v >= 0 && v <= 360 || 'Диапазон: 0-360'
                ]"
              ></v-text-field>
            </v-col>
            <v-col cols="6">
              <v-text-field
                v-model.number="formData.parameters.altitude"
                label="Высота солнца (°)"
                type="number"
                min="0"
                max="90"
