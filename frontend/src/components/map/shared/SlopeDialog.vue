...
async function runAnalysis() {
  ...
  try {
    ...
    console.error('Failed to run slope analysis:', err);
  } finally {
    loading.value = false;
  }
}
  ...
  <v-dialog v-model="internalShow" max-width="500px">
    <v-card>
      <v-card-title class="pa-4 bg-primary text-white">
        <v-icon start icon="mdi-slope-uphill"></v-icon>
        Расчет крутизны уклонов
      </v-card-title>
      ...
      <v-card-text class="pa-4">
        <v-form ref="form" v-model="valid">
          ...
          <v-select
            v-model="formData.parameters.units"
            :items="[
              { title: 'Градусы (0-90°)', value: 'degrees' },
              { title: 'Проценты (%)', value: 'percent' }
            ]"
            label="Единицы измерения уклона"
            variant="outlined"
            density="comfortable"
            required
          ></v-select>
