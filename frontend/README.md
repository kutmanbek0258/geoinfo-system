...
- **Интерактивная карта:** Визуализация и редактирование векторных данных (точки, линии, полигоны) с использованием OpenLayers.
- **Управление проектами:** Создание, редактирование и совместное использование (sharing) проектов.
- **Импорт KML:** Удобный интерфейс для загрузки KML-файлов, позволяющий быстро создавать новые проекты на основе существующих геоданных.
- **Работа с документами:** Интеграция с OnlyOffice для онлайн-редактирования документов, привязанных к объектам на карте.
- **Слои аэрофотосъемки:** Поддержка отображения растровых слоев (WMS) из GeoServer с управлением прозрачностью.
- **3D-визуализация и рельеф:** Полноценный 3D-режим на базе **CesiumJS**, позволяющий просматривать карту с учетом реального ландшафта (Terrain) и создавать объекты с привязкой к высоте.
- **Видеотрансляции:** Просмотр живых потоков с камер (HLS) непосредственно через интерфейс карты.

## 3D Возможности (CesiumJS)

Система поддерживает переключение между 2D (OpenLayers) и 3D (CesiumJS) режимами отображения.

### Основные функции 3D:
- **Поддержка Terrain:** Визуализация глобального рельефа (Cesium World Terrain) и пользовательских слоев ландшафта, загруженных через `terrain-service`.
- **Создание 3D-объектов:** Инструменты рисования точек, линий и полигонов непосредственно на 3D-поверхности с автоматической привязкой к рельефу (clamping to ground).
- **Интерактивное редактирование:** Режим изменения геометрии в 3D с использованием контрольных точек (draggers) для перемещения вершин объектов.
- **Управление слоями:** Возможность наложения WMS-слоев поверх 3D-рельефа с настройкой их прозрачности.

Интеграция реализована в компоненте `CesiumMapComponent.vue`, который взаимодействует с `terrain-service` для получения доступных слоев ландшафта и использует Vuex для синхронизации геопространственных данных между 2D и 3D режимами.
