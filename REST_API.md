...
Сервис для управления задачами сложной обработки геоданных.
...
### Sentinel-2 (`/api/geo-abstraction/sentinel`)
- `POST /upload` - Загрузить `.SAFE.zip` и создать задачу `SENTINEL_COG`. Параметры: `name`, `file`, `channels`.

### Слои (`/api/geo-abstraction/layers`)
- `GET /` - Список готовых 3D слоев.
