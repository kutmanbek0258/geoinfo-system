...
Сервис для централизованного управления растровыми данными, рельефом и интеграции с TiTiler.
...
- `POST /` - **Authority: `IMAGERY_LAYER_CREATE`**: Сохранить метаданные нового слоя. Принимает `projectId` в теле запроса.
...
- `GET /page-query` - **Authority: `IMAGERY_LAYER_READ`**: Получить список слоев с пагинацией. Поддерживает фильтр `?projectId=<uuid>`. Возвращает (Слои проекта + Общие слои).
- `POST /upload` - **Authority: `IMAGERY_LAYER_CREATE`**: Загрузить произвольный GeoTIFF для оптимизации (COG) и авто-публикации. Параметры: `name`, `file`, `projectId` (optional).

### Стили растра (`/api/geo-abstraction/raster-style`)
- `GET /` - **Authority: `IMAGERY_LAYER_READ`**: Получить все стили (системные и пользовательские).
- `GET /{id}` - **Authority: `IMAGERY_LAYER_READ`**: Получить конкретный стиль по ID.
- `POST /` - **Authority: `IMAGERY_LAYER_CREATE`**: Создать новый пользовательский стиль.
- `PUT /{id}` - **Authority: `IMAGERY_LAYER_UPDATE`**: Обновить существующий пользовательский стиль.
- `DELETE /{id}` - **Authority: `IMAGERY_LAYER_DELETE`**: Удалить пользовательский стиль (системные стили защищены от удаления).
...
- `GET /` - Список готовых 3D слоев рельефа. Поддерживает фильтр `?projectId=<uuid>`.
